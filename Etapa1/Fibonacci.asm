...
main:
		la			$s0, sequencia		# carrega o endereço de sequenacia
		la			$s1, size			# carrga o endereço de size
		lw			$s1,0($s1)			# recupera o vaor de size que é a quantidade de numeros a ser gerados pelo fibonaccp
		addiu		$t1,$zero,0			# inicia o $t0 e $t1 com os primeiros valores da sequencia
		addiu		$t2,$zero,1			# 
		sw			$t1,0($s0)			# salva o  primeiro valor da sequencia que é 0
loop:
